# Wiki Overview

> 마지막 갱신: 2026-06-13
> 이 페이지는 `/ingest` 실행 시 자동으로 업데이트됩니다.

## 위키 현황

- 총 소스: 3개
- 추출된 엔티티: 1개
- 추출된 컨셉: 11개
- 저장된 답변: 0개

## 핵심 주제

1. **팀 Git 운영 규칙** — 브랜치 전략, 커밋 메시지, PR 작성, 머지 방식
2. **마냑 서비스 기획** — AI 인터랙티브 스토리 서비스, 3단계 제작 모드, 스토리 탐색, 채팅 플레이, 초기 KPI 구조
3. **작업 유형별 PR 템플릿** — Web/Mobile, Backend, AI, Infra 변경에 맞춘 PR 본문 구조와 검증 체크리스트

## 주요 엔티티

- [[entities/마냑]] — AI 인터랙티브 스토리 서비스, 3단계 제작·스토리 탐색·채팅 플레이·운영 KPI

## 주요 컨셉

- [[concepts/Git-브랜치-전략]] — main/dev 기반 브랜치 운영 전략
- [[concepts/커밋-메시지-컨벤션]] — Jira 이슈 번호 연동 커밋 메시지 형식
- [[concepts/브랜치-이름-규칙]] — 태그/KNK-이슈번호 형식의 브랜치 네이밍 규칙
- [[concepts/PR-규칙]] — PR 제목·본문·태그 작성 기준과 템플릿 기반 검증 항목
- [[concepts/PR-템플릿]] — 작업 유형별 PR 본문 섹션과 작성 항목
- [[concepts/PR-검증-체크리스트]] — 테스트·보안·문서·영역별 변경 영향 점검 항목
- [[concepts/머지-전략]] — 브랜치 목적별 Squash/Merge Commit/Rebase 적용 규칙
- [[concepts/인터랙티브-스토리텔링]] — AI와 사용자가 협력해 이야기를 만드는 콘텐츠 형태
- [[concepts/스토리-제작-모드]] — 숙련도별 간편/일반/전문가 3단계 스토리 제작 패턴
- [[concepts/크레딧-시스템]] — LLM 비용을 가상 화폐로 추상화하는 AI 서비스 과금 모델
- [[concepts/AI-인터랙티브-스토리-KPI]] — AI 인터랙티브 스토리 서비스의 초기 성장·참여·완료율 지표

## 최근 인사이트

Pull Request 템플릿은 하나의 PR 규칙을 모든 작업에 일괄 적용하지 않고, Web/Mobile, Backend, AI, Infra의 변경 위험을 나누어 화면·API·AI 평가·Infra 검증 항목을 각 템플릿에 직접 포함한다.
