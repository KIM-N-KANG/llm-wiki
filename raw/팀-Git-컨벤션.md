...
- **[태스크] `KNK-15` 로그인 관련 DB 인덱싱 및 성능 최적화**
    - **[서브 태스크] `KNK-16` User 테이블 이메일 칼럼 인덱스 추가**
        - Branch: `KNK-16_chore-add_email_index_to_user_table`
...
```
dev
└── KNK-12_feat-add_google_login_button_ui
```
...
```
KNK-12_feat-add_google_login_button_ui → dev
```
...
```
release/v1.0.0
└── KNK-20_fix-fix_login_redirect_error
```
...
```
KNK-20_fix-fix_login_redirect_error → release/v1.0.0
```
...
dev
 ...
 │        └── Squash and Merge
 │
 ├── KNK-13_feat-integrate_google_login_api
 │    └── PR: [KNK-13] Feat → dev
 │        └── Squash and Merge
 │
 └── release/v1.0.0
      ├── KNK-20_fix-fix_login_redirect_error
      │    └── PR: [KNK-20] Fix → release/v1.0.0
      │        └── Squash and Merge
      │
